...
(function ($) {
    ...
    $.fn.endlessPaginate = function(options) {
        var defaults = {
            ...
            // Search selector
            searchSelector: '.endless_search_link',
            // Search pattern
            searchInput: '.endless_search_input',
        ...
        let getContext = function(onclick_link, input_link) {
            return {
                key: onclick_link.data("el-querystring-key").split(' ')[0],
                url: onclick_link.attr('href'),
                pattern: input_link.val()
            };
        };
        ...
        return this.each(function() {
            ...
            element.on('click', settings.pagesSelector, function() {
                let onclick_link = $(this),
                    input_link = $(settings.searchInput),
                    html_link = onclick_link.get(0),
                    context = getContext(onclick_link, input_link);
                ...
                // Fire onClick callback.
                if (settings.onClick.apply(html_link, [context]) !== false) {
                    let page_template = onclick_link.closest(settings.pageSelector),
                        data = 'querystring_key=' + context.key + "&pattern=" + context.pattern;
